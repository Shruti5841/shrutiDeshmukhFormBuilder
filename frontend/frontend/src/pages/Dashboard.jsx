...
const Dashboard = () => {
  ...
  const [completionRate, setCompletionRate] = useState(0); // Initialize to 0
  ...
  const handleFormCompletion = (isCompleted) => {
    if (isCompleted) {
      setCompletionRate(100); // Set to 100% when completed
    } else {
      setCompletionRate(0); // Reset to 0% if not completed
    }
  };
  const handleSubmit = async (formData) => {
    // Submit form logic...
    const isCompleted = true; // Determine if the form is completed
    handleFormCompletion(isCompleted);
  };
